...
@pytest.mark.parametrize(
    ...
    [
        ([0.5, 0.5, 0.5, 0.6], 0.01, 1, 1000, 20, 0),
        ([0.1, 0.5, 0.3, 0.6], 0.01, 1000, 1000, 20, 1),
    ]
)
def test__variable_step_dataset_with_norm_step__produces_same_samples_as_sim(
    ...
    simulator = lotka_volterra.LotkaVolterraSimulator(*simulator_params, time_delta=time_delta)
    dataset = lotka_volterra.VariableStepLotkaVolterraDataset(
        simulator, size=200, min_step_delta=min_step, max_step_delta=max_step,
        samples_per_sim=40, subsample_sim=subsample_sim, seed=seed, norm_step=True)
    for i in range(len(dataset)):
        x0 = dataset.x[i]
...
@pytest.mark.parametrize(
    'simulator_params, time_delta, step_delta, subsample_sim, seed',
    [
        ([0.5, 0.5, 0.5, 0.6], 0.01, 300, 4, 0),
        ([1.5, 2.5, 0.5, 0.6], 0.01, 1, 5, 1),
        ([0.5, 0.4, 0.5, 0.6], 0.001, 1000, 1, 2),
        ([2.5, 3.4, 2.5, 3.6], 0.009, 10, 200, 4),
    ]
)
def test__fixed_step_dataset__produces_same_samples_as_sim(simulator_params, time_delta, step_delta, subsample_sim,
                                                           seed):
    simulator = lotka_volterra.LotkaVolterraSimulator(*simulator_params, time_delta=time_delta)
    dataset = lotka_volterra.FixedStepLotkaVolterraDataset(
        simulator, size=500, step_delta=step_delta, subsample_sim=subsample_sim, seed=seed)
    assert len(dataset.x) == dataset.size
    for i in range(len(dataset)):
        x0 = dataset.x[i]
        sim_data = simulator.run_simulation(x0[0], x0[1], step_delta)
